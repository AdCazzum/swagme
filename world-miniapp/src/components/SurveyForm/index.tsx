...
export const SurveyForm: React.FC<SurveyFormProps> = ({ formId, onBack }) => {
  ...
  const validateForm = (): boolean => {
    ...
    return questions.every((question, i) => 
    ...
    return (
      <div className="flex flex-col gap-4 rounded-xl w-full border-2 border-gray-200 p-4">
        <div className="flex items-center justify-between">
          ...
          <Button onClick={onBack} variant="tertiary" size="sm">Back</Button>
        </div>
  ...
  if (error && !formInfo) {
    return (
      <div className="flex flex-col gap-4 rounded-xl w-full border-2 border-gray-200 p-4">
        ...
        <Button onClick={() => window.location.reload()} variant="primary" size="lg" className="w-full">
          Try Again
        </Button>
      </div>
    );
    ...
    <div className="flex flex-col gap-6 rounded-xl w-full border-2 border-gray-200 p-4">
      ...
      {formInfo && (
        <div className="bg-blue-50 border-2 border-blue-200 rounded-xl p-4">
          <h2 className="text-xl font-bold text-blue-900 mb-2">{formInfo.title}</h2>
          ...
          <div className="flex items-center gap-4 text-sm text-blue-600">
            ...
            <span className={`px-2 py-1 rounded-full text-xs ${
              formInfo.isActive ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"
            }`}>
      ...
      {(isSubmitting || isConfirming || isConfirmed || transactionId) && (
        <div className="bg-blue-50 border-2 border-blue-200 rounded-xl p-4">
          ...
          <p className="text-sm text-blue-700 mb-2">
            {isSubmitting ? "Please confirm the transaction in World App" :
             isConfirming ? "Transaction is being confirmed on blockchain..." :
             isConfirmed ? "Your survey response has been recorded on blockchain!" : "Transaction ID received"}
          </p>
          {transactionId && (
            <div className="mt-3 p-3 bg-white rounded-lg border border-blue-200">
              <p className="text-xs font-medium text-blue-800 mb-1">Transaction ID:</p>
              <p className="text-xs text-blue-600 font-mono break-all">{transactionId}</p>
              {isConfirmed && (
                <p className="text-xs text-green-600 mt-2 font-medium">✅ Confirmed on blockchain</p>
              )}
            </div>
          )}
        </div>
      ...
      <div className="space-y-6">
        {questions.map((question, index) => (
          <div key={index} className="bg-white border-2 border-gray-200 rounded-xl p-4 shadow-sm">
      ...
      <div className="sticky bottom-0 bg-white pt-4 border-t border-gray-200">
        <LiveFeedback
          label={{
            failed: hasError ? errorMessage || "Transaction failed" : "Submit failed",
            pending: isConfirming ? "Confirming on blockchain..." : 
                     isSubmitting ? "Processing transaction..." : "Processing...",
            success: isConfirmed ? "Transaction confirmed on blockchain!" : "Submitted successfully!",
          }}
          state={hasError ? "failed" : 
                 (isSubmitting || isConfirming) ? "pending" : 
                 isConfirmed ? "success" : undefined}
          className="w-full"
        >
          <Button
          ...
          >
            {isSubmitting ? "Submitting..." : 
             ...
             isConfirmed ? "Confirmed!" : "Submit to Blockchain"}
          </Button>
        </LiveFeedback>
        {questions.some((q) => q.isRequired) && (
          <p className="mt-2 text-xs text-gray-500 text-center">* Required fields to complete</p>
        )}
      </div>
    </div>
  );
